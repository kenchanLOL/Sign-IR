...
class Attention(nn.Module):
    def __init__(self, dim, heads=16, dim_head=64, attn_drop=0.0):
        super().__init__()
    ...
    def forward(self, x, mask=None):
        ...
        if mask is not None:
            mask = F.pad(mask.flatten(1), (1, 0), value=1.0)  # [b, 64] --> [b, 65]
...
class CrossAttention(nn.Module):
    def __init__(
        self, dim, heads=8, qkv_bias=False, qk_scale=None, attn_drop=0.0, proj_drop=0.0
    ):
        super().__init__()
        num_heads = heads
...
class Cross_att_layer(nn.Module):
    def __init__(
        self, dim=1024, heads=16, depth=2, dropout=0.1, attn_drop=0.0, mlp_dim=768
    ):
            ...
            self.layers.append(
                nn.ModuleList(
                    [
                        ...
                        PreNorm(
                            dim, CrossAttention(dim, heads=heads, attn_drop=attn_drop)
                        ),
                        PreNorm(dim, FeedForward(dim, mlp_dim, dropout=dropout)),
                    ]
                )
...
def config_decoder(config):
    ...
    if decoder_type == "LD":
        return MBartForConditionalGeneration.from_pretrained(
            ...
            config=Path(config["model"]["visual_encoder"]) / "config.json",
        )
    elif decoder_type == "LLMD":
        return MBartForConditionalGeneration.from_pretrained(
            config["model"]["transformer"],
            ignore_mismatched_sizes=True,
            config=Path(config["model"]["transformer"]) / "LLMD_config.json",
        )
...
class gloss_free_model(nn.Module):
    ...
    def share_forward(self, src_input):
        ...
        frames_feature, image_feats = self.backbone(
            src_input["input_ids"].cuda(), src_input["src_length_batch"]
        )
        attention_mask = src_input["attention_mask"]
    ...
    def forward(self, src_input, tgt_input):
        ...
        inputs_embeds, attention_mask, frames_feature = self.share_forward(
            src_input
        )  # inputs_embeds : B, L, C
...
class VideoTokenMLP(nn.Module):
    def __init__(self, config, output_dim):
        super().__init__()
        input_dim = config.get("model", {}).get("input_dim", 1024)
        self.linear1 = nn.Linear(input_dim, config["model"]["hidden_size"])
        self.LayerNorm = nn.LayerNorm(config["model"]["hidden_size"])
        self.activation = nn.ReLU()
        self.linear2 = nn.Linear(config["model"]["hidden_size"], output_dim)
...
class ImageCLIP_IR(nn.Module):
    def __init__(self, config, inplanes=1024, planes=1024, head_type="linear"):
        super(ImageCLIP_IR, self).__init__()
        self.config = config
        ...
        self.lm_head = make_head(inplanes, planes, head_type)
        # self.projection = VideoTokenMLP(config, planes)
    ...
    def forward(self, src_input):
        ...
        if self.cls_token is not None:
            ...
            output = self.lm_head(outs["last_hidden_state"].mean(dim=1))
        return output, images


class TextCLIP_IR(nn.Module):
    def __init__(self, config=None, inplanes=1024, planes=1024, head_type="identy"):
        super(TextCLIP_IR, self).__init__()
    ...
    def forward(self, tgt_input):
        ...
        if self.use_avg:
            masked_logits = txt_logits * tgt_input["attention_mask"].unsqueeze(-1).cuda()
            # Sum the masked logits and divide by the sum of the attention mask to get the mean
            sum_masked_logits = masked_logits.sum(dim=1)
            output = sum_masked_logits / tgt_input["attention_mask"].sum(dim=1, keepdim=True).cuda()
...
class VideoCLIP(nn.Module):
    def __init__(self, config, embed_dim=1024):
        ...
        self.model_txt = TextCLIP_IR(config, inplanes=embed_dim, planes=embed_dim)
        self.model_images = ImageCLIP_IR(config, inplanes=embed_dim, planes=embed_dim)
