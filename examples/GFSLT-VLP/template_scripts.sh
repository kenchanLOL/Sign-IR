...
# CUDA_VISIBLE_DEVICES=0,1 torchrun --nproc_per_node=2 --master_port=1236 train_vlp_SignCL.py \
#     --batch-size 4 --epochs 80 --opt sgd --lr 0.01 \
#     --output_dir out/0626_csl_VLP_SignCL --training-refurbish True \
#     --noise-rate 0.15 --noise-type omit_last --random-shuffle False \
#     --decoder-type LLMD --config ./configs/config_gloss_free.yaml
...
#     train_dsl_SignCL.py --batch-size 2 --epochs 0 --opt sgd --lr 0.0065 \
#     --output_dir out/0630_GF_SignCL --config ./configs/config_gloss_free.yaml \
#     --resume out/0630_GF_SignCL/best_checkpoint.pth --decoder-type LD --eval

CUDA_VISIBLE_DEVICES=0 torchrun --nproc_per_node=1 --master_port=1036 \
    eval_dsl_SignCL.py --batch-size 2 \
    --output_dir out/0630_GF_SignCL --config ./configs/config_gloss_free.yaml \
    --resume out/0630_GF_SignCL/best_checkpoint.pth --decoder-type LD
