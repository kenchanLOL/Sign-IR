...
def main(args, config):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train_stats = train_one_epoch(args, model, criterion, train_dataloader, optimizer, device, epoch, config, PAD_IDX, loss_scaler, TD_train_dict, gradient_accumulation_steps = config['training']['gradient_accumulation_steps'])
...
def train_one_epoch(args, model: torch.nn.Module, criterion: nn.CrossEntropyLoss,
    ...
    for step, (src_input, tgt_input, masked_tgt_input) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        with torch.cuda.amp.autocast():
            logits_per_image, logits_per_text, ground_truth, _ = model(src_input, tgt_input)
