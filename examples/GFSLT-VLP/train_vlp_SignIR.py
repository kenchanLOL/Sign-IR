...
def get_args_parser():
    ...
    parser.add_argument('--loss_fn', default='KL', type=str, choices=['KL', 'CL'])
...
def main(args, config):
    ...
    print("non distributed verison ready")
    ...
    print("text_decoder ready")
    lr_scheduler_td = scheduler.CosineAnnealingLR(
        optimizer=optimizer_td,
        eta_min=1e-8,
        T_max=args.epochs,
    )
    print("lr_scheduler_td ready")
    TD_train_dict = dict(
        optimizer=optimizer_td,
        lr_scheduler=lr_scheduler_td,
        text_decoder=text_decoder
    )
    print("TD_train_dict ready")
    if args.loss_fn == "KL":
        criterion = utils.KLLoss()
    elif args.loss_fn == "CL":
        criterion = utils.ClipLoss()
...
def train_one_epoch(args, model: torch.nn.Module, criterion: nn.CrossEntropyLoss,
    ...
    for step, (src_input, tgt_input, masked_tgt_input) in enumerate(
        ...
        with torch.cuda.amp.autocast():
            logits_per_image, logits_per_text, logit_scale, ground_truth, frames_feature = model(src_input, tgt_input)
            if args.loss_fn == "KL":
                # print(logits_per_image.shape, logits_per_text.shape, ground_truth.shape)
                logits_per_image_ = logit_scale * logits_per_image @ logits_per_text.t()
                logits_per_text_ = logit_scale * logits_per_text @ logits_per_image.t()
                loss_imgs = loss_img(logits_per_image_, ground_truth)
                loss_texts = loss_txt(logits_per_text_, ground_truth)
                ml_loss = (loss_imgs + loss_texts) / 2.
            elif args.loss_fn == "CL":
                ml_loss = loss_clip(logits_per_image, logits_per_text, logit_scale)
            total_loss = ml_loss
        ...
        if step % 5 == 0:
            ...
            if args.debug:
                model_txt = model.model_txt
            else:
                model_txt = model.module.model_txt
            with torch.cuda.amp.autocast():
                lm_logits = TD_train_dict['text_decoder'](tgt_input, masked_tgt_input, model_txt)
        ...
        metric_logger.update(ml_loss=ml_loss.item())
        if not args.ignore_signcl:
...
def main_extract_features(args, config):
    ...
    # Model setup
    print("Loading model...")
    model = VideoCLIP(config=config).to(device)
    if args.finetune:
        checkpoint = torch.load(args.finetune, map_location='cpu')
...
def setup_run(args, config):
    if args.log_all:
        ...
        if utils.is_main_process():
            ...
            run.define_metric("training/*", step_metric="epoch")
            run.define_metric("dev/*", step_metric="epoch")
            run.name = args.output_dir.split('/')[-1] + f"_{args.batch_size}"
        else:
