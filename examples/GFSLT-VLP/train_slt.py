...
def main(args, config):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train_stats = train_one_epoch(args, model, criterion, train_dataloader, optimizer, device, epoch, config, loss_scaler, mixup_fn, gradient_accumulation_steps = config['training']['gradient_accumulation_steps'])
...
def train_one_epoch(args, model: torch.nn.Module, criterion: nn.CrossEntropyLoss,
                    ...
                    set_training_mode=True, gradient_accumulation_steps: int = 1):
    ...
    for step, (src_input, tgt_input, masked_tgt_input) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        out_logits, _, _ = model(src_input, tgt_input)
...
def evaluate(args, dev_dataloader, model, model_without_ddp, tokenizer, criterion,  config, UNK_IDX, SPECIAL_SYMBOLS, PAD_IDX, device):
    ...
    with torch.no_grad():
        ...
        for step, (src_input, tgt_input, masked_tgt_input) in enumerate(metric_logger.log_every(dev_dataloader, 10, header)):
